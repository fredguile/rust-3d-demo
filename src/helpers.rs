...
pub fn create_memory_buffer() -> Result<Memory, JsValue> {
    Ok(wasm_bindgen::memory().dyn_into::<WebAssembly::Memory>()?)
}

pub trait TypedArray {
    fn new(memory_buffer: &JsValue) -> Self;
    fn subarray(self: &Self, location: u32, length: u32) -> Self;
}
...
impl TypedArray for Float32Array {
    ...
    fn subarray(self: &Self, location: u32, length: u32) -> Self {
        self.subarray(location, length)
    }
}

impl TypedArray for Uint16Array {
    fn new(memory_buffer: &JsValue) -> Self {
        Uint16Array::new(&memory_buffer)
    }
...
pub fn create_typed_array<T: TypedArray>(memory_buffer: Memory, location: u32, offset: usize) -> T {
    T::new(&memory_buffer.buffer()).subarray(location, location + offset as u32)
}
